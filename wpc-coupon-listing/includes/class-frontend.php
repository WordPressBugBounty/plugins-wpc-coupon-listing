    ...
    class Wpccl_Frontend {
        ...
        public function enqueue_scripts() {
            ...
            wp_localize_script( 'wpccl-frontend', 'wpccl_vars', [
                    ...
                    'active_in'   => Wpccl_Helper::localization( 'active_in', /* translators: time */ esc_html__( 'Active in %s', 'wpc-coupon-listing' ) ),
        ...
        public function list_shortcode( $attrs ) {
            ...
            return '<div class="wpccl-coupons wpccl-coupons-shortcode ' . esc_attr( $attrs['extra_class'] ) . '">' . self::render_coupons( wc_string_to_bool( $attrs['plain'] ), $attrs['hide'] ) . '</div>';
        }
        ...
        public function render_coupons( $plain = false, $hide = '' ) {
            ...
            if ( ! empty( $hide ) ) {
                $hide_arr = array_map( 'trim', explode( ',', $hide ) );
            } else {
                $hide_arr = [];
            }
            ...
            foreach ( $coupons as $coupon_id => $coupon_data ) {
                ...
                $expiry      = ! empty( $expiry_date ) ? sprintf( Wpccl_Helper::localization( 'expires', /* translators: date */ esc_html__( 'Expires on: %s', 'wpc-coupon-listing' ) ), $expiry_date ) : Wpccl_Helper::localization( 'never_expire', esc_html__( 'Never expire', 'wpc-coupon-listing' ) );
            ...
            if ( ! empty( $coupon_ids ) ) {
                ...
                foreach ( $coupon_ids as $coupon_id ) {
                    ...
                    if ( $minimum_amount > 0 && apply_filters( 'woocommerce_coupon_validate_minimum_amount', $minimum_amount > $cart_subtotal, $coupon, $cart_subtotal ) ) {
                        $coupon_enable  = false;
                        $coupon_message = sprintf( Wpccl_Helper::localization( 'minimum_spend', /* translators: minimum */ esc_html__( 'The minimum spend for this coupon is %s.', 'wpc-coupon-listing' ) ), wc_price( $minimum_amount ) );
                    }
                    ...
                    if ( $maximum_amount > 0 && apply_filters( 'woocommerce_coupon_validate_maximum_amount', $maximum_amount < $cart_subtotal, $coupon ) ) {
                        $coupon_enable  = false;
                        $coupon_message = sprintf( Wpccl_Helper::localization( 'maximum_spend', /* translators: maximum */ esc_html__( 'The maximum spend for this coupon is %s.', 'wpc-coupon-listing' ) ), wc_price( $maximum_amount ) );
                    }
